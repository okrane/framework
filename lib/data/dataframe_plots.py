...
def stackbar(data,
    ...
    # -- uni_gvar
    if is_gvar_datetime:
        uni_gvar=np.unique([x.to_datetime() for x in data[gvar]]).tolist()
        if len(uni_gvar)>1:
    ...
    for idx_gvar in range(0,len(uni_gvar)):
        ...
        if add_gvar_txt:
            if is_horizontal:
                plt.text(tmp_cum_value*1.02+2, idx_gvar + 0.33, str(round((tmp_cum_value*1.02)*100)/100),
    ...
    if not is_gvar_datetime:
        ...
        if not is_horizontal:
            ylim[1]*=VALS_MULTIPLIER
            xlim=[datetime2matlabdn(first_dt_uni_gvar),datetime2matlabdn(uni_gvar[-1])]
        else:
            xlim[1]*=VALS_MULTIPLIER
            ylim=[datetime2matlabdn(first_dt_uni_gvar),datetime2matlabdn(uni_gvar[-1])]
...
if __name__=='__main__':
    ...
    data['end_slice']=[dt.datetime.combine(max_day,x.time()) for x in data.index]
