...
def storeDB(l_orders, Collection, client):
    
    database = client['DB_test']
    collection = database[Collection]
    
    print "insert ClientOrders into database"
    collection.insert(l_orders, manipulate=False)
...
def OrderLife(order, dico_fix, day, ignore_tags, dico_tags, server):
    ...
    if Type == 'D':
        ...
        for str_msg in stdout_grep:
            ...
            p_real_duration = time.mktime(p_msg['SendingTime'].timetuple()) - time.mktime(s_time.timetuple())
            global_duration = p_real_duration
        ...
        if not done and not replaced :
            ...
            while not done and loop_count<100 and replaced == True:
                ...
                for str_msg in stdout_grep:
                    ...
                    real_duration = time.mktime(cc_msg['SendingTime'].timetuple()) - time.mktime(s_time.timetuple())
                ...
                global_duration += real_duration
                if not done and not replaced :
                    reason = 'Front End handling'
                
                c_msg.update({'reason': reason, 'nb_exec' : nb_exec, 'exec_qty': exec_qty, 'duration': real_duration, 'occ_ID': p_ClOrdID})
                order_life.append(c_msg)
        
        order.update({'reason': p_reason, 'nb_exec' : p_nb_exec, 'nb_replace' : nb_replace, 'exec_qty': p_exec_qty, 'duration': p_real_duration, 'occ_duration': global_duration, 'occ_ID': p_ClOrdID})
...
if __name__ == '__main__':
    ...
    ignore_tags = [8, 21, 22, 9, 34, 49, 56, 10, 47]
...
#     database = Client['DB_test']
#     collection = database['StreetOrders']
#     collection.remove()
