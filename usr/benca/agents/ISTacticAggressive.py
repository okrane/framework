...
class AggressiveTacticIS:
    
    def __init__(self, ISManager, use_business_time):
    ...
    def send_order(self, lob):
        ...
        check_time = self._prvt__check_time()
        if not check_time:
            return
        # order size
        MaxQtyAvailable = 0
        ...
        if self.B['limit_price'] >= 0.0:
            if self.B._side == 1:
                if lob.getAsk(0).price > self.B['limit_price']:
                    self.cancel_order()
                    return
            else:
                if lob.getBid(0).price < self.B['limit_price']:
                    self.cancel_order()
                    return
        # create order
        self.cancel_order()
