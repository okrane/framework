...
class BestExecGLFT(BasePlotAgent):
    ...
    def process(self, event):
        ...
        if code == 1:
            ...
            if flag_new_slice or mv['flag_exec'] or (time_num > mv['last_order_time'] + self['max_exec_time']):
                ...
                round_delta = round(delta_t_q)
                price = fd['BEST_BID1']+min(round_delta,spread+self['max_order_price'])*mv['tick_size']
    ...
    def last_process(self):
        ...
        f = open('C:/output_test_round.csv', 'a')
        f.write(str(self['security_id'])+';'+str(self['date'])+';'+str(self['delta_target'])+';'+str(self['ref_order_size'])+';'+str(self['asked_qty_multiple'])+';'+str(self['ewma_theta'])+';'+str(self['ewma_theta_ats'])+';'+str(self['max_exec_time'])+';'+str(cp['gamma'])+';'+str(imp_shortfall)+';'+str(performance)+';'+str(mv['percentage_aggressive'])+'; ;')
        for p in mv['participation']:
            f.write(str(p)+';')
        f.write('\n');
        f.close()
    ...
    def update_participation(self,fd,mv,time_num):
        ...
        mv['executed_volume_since_last_stat'] = 0
        mv['volume_at_start'] = fd['VOLUME']
        mv['participation'].append(participation)
