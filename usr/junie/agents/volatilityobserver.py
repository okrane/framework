...
class VolatilityObserver(BaseAgent):    
    ...
    def public_parameters():
        parameters = {'num_trades' : {'label' : 'number of trades for Garman-Klass volatility', 'value' : 1000},
                      'start_time' : {'label' : 'start time of observe', 'value' : 8*3.6e9}}
    ...
    def __init__(self, setup, context, parameters, trace):
        ...
        #parameter @start_time: start time of observe
    ...
    def compute_normalized_volatility(self, volatility, dt):  
        ...
        if volatility != None and dt > EPSILON:
            return 1e4 * volatility * sqrt(10 / dt) 
        else:
            return None
            ...
            if self.update(event) == 1:      
                ...
                if feed['LIT_TRADE_EVENT']:
                    ...
                    for i in feed['LIT_LAST_DEALS_IDXS']:
                        ...
                        #if the trade time is early than the start time, do nothing
                        if time < self.start_time:
                            return
                        #if the trade time >= start time, start to compute the volatility
                        trades_times.append(time)
                        trades_prices.append(price)
                       
                    if len(trades_times) < self.num_trades:
                        return
                    #slide the left window when there are more than @num_trades trades
                    while len(trades_times) > self.num_trades:
                        trades_times.pop(0)
                        trades_prices.pop(0)       
                    ...
                    dt = float(gk['close_time'] - gk['open_time']) / (6e7)
