...
def get_runs_by_perimeters(perimeter_ids, estimator_id, context_id):
    ids = Connections.exec_sql("QUANT",
                                ''' SELECT ds.security_id, er.stamp_date, lr.last_run_id, run_quality FROM %s..perimeter_domain pd
                                    JOIN %s..domain_security ds on ds.domain_id=pd.domain_id
                                    ...
                                    JOIN %s..last_run lr on lr.job_id=a.job_id and lr.context_id = a.context_id
                                    JOIN %s..estimator_runs er on er.run_id=lr.last_run_id
                                    WHERE pd.perimeter_id in (%s)
                                ''' % (bq, bq, bq, estimator_id, context_id, bq, bq, ','.join(map(str,perimeter_ids)))
                            )
